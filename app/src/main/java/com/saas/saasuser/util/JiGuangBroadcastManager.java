...
public final class JiGuangBroadcastManager {
    ...
    private static JiGuangBroadcastManager mInstance;

    public static JiGuangBroadcastManager getInstance(Context context) {
        Object var1 = mLock;
        synchronized (mLock) {
            if (mInstance == null) {
                mInstance = new JiGuangBroadcastManager(context.getApplicationContext());
            }
    ...
    private JiGuangBroadcastManager(Context context) {
        this.mAppContext = context;
        this.mHandler = new Handler(context.getMainLooper()) {
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case 1:
                        JiGuangBroadcastManager.this.executePendingBroadcasts();
                        break;
                    default:
    ...
    public boolean sendBroadcast(Intent intent) {
        ...
        synchronized (this.mReceivers) {
            ...
            if (debug) {
                Log.v("JiGuangBroadcastManager", "Resolving type " + type + " scheme " + scheme + " of intent " + intent);
            }

            ArrayList entries = (ArrayList) this.mActions.get(intent.getAction());
            if (entries != null) {
                if (debug) {
                    Log.v("JiGuangBroadcastManager", "Action list: " + entries);
                }
                ...
                for (i = 0; i < entries.size(); ++i) {
                    ...
                    if (debug) {
                        Log.v("JiGuangBroadcastManager", "Matching against filter " + receiver.filter);
                    }

                    if (receiver.broadcasting) {
                        if (debug) {
                            Log.v("JiGuangBroadcastManager", "  Filter\'s target already added");
                        }
                    } else {
                        int match = receiver.filter.match(action, type, scheme, data, categories, "JiGuangBroadcastManager");
                        if (match >= 0) {
                            if (debug) {
                                Log.v("JiGuangBroadcastManager", "  Filter matched!  match=0x" + Integer.toHexString(match));
                            }
                            ...
                            Log.v("JiGuangBroadcastManager", "  Filter did not match: " + reason);
                        }
                    }
                }
