...
public class RingStatisticsView extends View {
    ...
    private String[] mName=new String[]{"订单类型A","订单类型B","订单类型C","订单类型D"};
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCount; i++) {
            ...
            String percent = mName[i]+df.format(f1) + "%";
    ...
    public void setPercentAndColors(float[] percent, int[] colors,String[] names) {
        if (percent != null && colors != null) {
            if (percent.length == colors.length && percent.length==names.length) {
                mPercent = percent;
                mColors = colors;
                mName=names;
            } else {
