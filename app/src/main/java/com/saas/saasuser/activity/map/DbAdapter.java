...
public class DbAdapter {
	...
	private final static String DATABASE_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + "/recordPath";
	static final String DATABASE_NAME = DATABASE_PATH + "/" + "new_record.db";
	private static final int DATABASE_VERSION = 1;
	private static final String RECORD_TABLE = "new_record";
	private static final String RECORD_CREATE = "create table if not exists new_record("
			+ KEY_ROWID
	...
	public Cursor getall() {
		return db.rawQuery("SELECT * FROM new_record", null);
	}
	...
	public long createrecord(String distance, String duration,
                             ...
                             String endpoint, String date,String orderid) {
	...
	public List<PathRecord> queryRecordAll() {
		...
		while (allRecordCursor.moveToNext()) {
			...
			record.setOrderId(allRecordCursor.getString(allRecordCursor
					.getColumnIndex(DbAdapter.KEY_ORDER_ID)));
	 ...
	 * @return
	 */
	public PathRecord queryRecordById(int mRecordItemId) {//把查找条件替换成订单id即可
		...
		if (cursor.moveToNext()) {
			...
			record.setEndpoint(AMapUtil.parseLocation(cursor.getString(cursor
					.getColumnIndex(DbAdapter.KEY_END))));
			record.setOrderId(cursor.getString(cursor
					.getColumnIndex(DbAdapter.KEY_ORDER_ID)));
		}
		return record;
	}
	 ...
	 * @return
	 */
	public PathRecord queryRecordByOrderId(String  mOrderId) {//把查找条件替换成订单id即可
		String where = KEY_ORDER_ID + "=?";

		String[] selectionArgs = new String[] {mOrderId};
		LogUtils.e("KEY_ORDER_ID:"+selectionArgs);
		Cursor cursor = db.query(RECORD_TABLE, getColumns(), where,
				selectionArgs, null, null, null);
		PathRecord record = new PathRecord();
		if (cursor.moveToNext()) {
	...
	private String[] getColumns() {
		return new String[] { KEY_ROWID, KEY_DISTANCE, KEY_DURATION, KEY_SPEED,
				KEY_LINE, KEY_STRAT, KEY_END, KEY_DATE,KEY_ORDER_ID };
	}
}
